...
public class CustomProgress extends View {
    ...
    //圆点指示器的半径
    private int mCircleRadius = DensityUtil.dip2px(12);
    ...
    private void initPaint() {
        ...
        bgPaint.setStrokeWidth(DensityUtil.dip2px(3));//大小为3dp转px
        ...
        progressPaint.setStrokeWidth(DensityUtil.dip2px(3));//大小为3dp转px

        //圆点指示器
        circlePaint = new Paint();
        circlePaint.setAntiAlias(true);//设置抗锯齿
        circlePaint.setColor(Color.parseColor("#fafafa"));//颜色
        circlePaint.setShadowLayer(DensityUtil.dip2px(2), 0, 0, Color.parseColor("#38000000"));//外阴影颜色
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int minHeight = mCircleRadius * 2 + (DensityUtil.dip2px(2) * 2);
...
//        L.d("onDraw");
    ...
    public void setProgress(int progress) {
        if (progress > 100 || progress < 0) {
            Toast.makeText(BaseApplication.getContext(), "输入的进度值不符合规范", Toast.LENGTH_SHORT).show();
        }
        setCurrentProgress();
        //设置当前进度的宽度
